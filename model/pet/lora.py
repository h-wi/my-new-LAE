...
class KVLoRA(nn.Module): # Key-Value LoRA
    def __init__(
        self,
        in_features,
        out_features,
    ...
    def forward(self, module: nn.Linear, input: torch.Tensor):
        items = zip(self.scale, self.lora_A, self.lora_B) # input: [1, 197. 768]
        weight = torch.cat([s(B @ A) for s, A, B in items], dim=0) # 1536x736
        zeros = torch.zeros_like(module.weight)[: -weight.shape[0]] # 768x768
        weight = torch.cat([zeros, weight], dim=0) # 2304x768, weight + zero
        output = F.linear(input, module.weight + weight, module.bias) # module.weight: [768, 2304] 사전 학습된 W_qkv
        return output # [1, 197, 2304]

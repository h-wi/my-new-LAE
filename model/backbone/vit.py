...
from .moe import ResidualAttentionBlock
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if moe:
            self.blocks = nn.Sequential(*[ResidualAttentionBlock(embed_dim, num_heads, expert) for _ in range(depth)])
        else:
...
def _load_weights(
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        block.attn.qkv.weight.copy_( #qkv
            torch.cat(
                [

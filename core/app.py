...
flags.DEFINE_string("cuda", "", "use cuda device")
flags.DEFINE_string('exp_name', "default", "set experience name")
...
class App:
    ...
    def main(self, argv):
        ...
        if not FLAGS.debug:
            wandb.init(project='my_moe',
                       name=FLAGS.exp_name,
                       config=config)
        else:
            config.module.log_wandb = False

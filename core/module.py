...
class Module(ModuleABC):
    ...
    def run(self, mode: str = "train"):
        ...
        for self.current_epoch in range(self.current_epoch, num_epochs):
            self.pre_train_epoch() # Set Online PET freeze or unfreeze
    ...
    def train_epoch(self, loader, num_batches):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            self.global_step += 1
            self.post_train_step() # EMA Update
        ...
        if self.cfg.log_wandb:
            wandb.log(
                {"epoch": self.current_epoch,
                 "Acc": self.val_acc.compute() * 100,
                 "Global Task Acc Avg": sum([acc.compute() * 100 for acc in self.val_task_accs]) / n_tasks
                 }
            )
